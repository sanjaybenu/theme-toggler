    ...
    <Grid container alignItems={"center"} justifyContent={"center"} sx={{textAlign:"center"}} >
      ...
      <Fade duration={3000} cascade={true} direction="up">
        ...
        <Typography variant="h4" >
          A <Typography variant="h4" sx={{color:"darkviolet"}}>Full</Typography> Stack Web Developer
        </Typography>
          ...
          <Typist typingDelay={300} loop={true} startDelay={5500}>
            {languages.map((language) => (
              <React.Fragment key={language}>
                <Typography variant="h3" sx={{color:"darkviolet"}}>{language}</Typography>

                <Typist.Backspace count={language.length} />
              </React.Fragment>
            ))}
            <Typography variant="h2" sx={{color:"darkblue", fontWeight:"700"}}>Welcome to my World </Typography>
